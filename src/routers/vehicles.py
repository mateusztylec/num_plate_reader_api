...
def get_vehicle_by_num_plate(num_plate: str, user = Security(get_user, scopes=[Role.ADMIN["name"], Role.USER["name"]]), db: Session = Depends(get_db)):
    '''
...
def get_vehicles(db: Session = Depends(get_db), user: int = Depends(get_user), limit: int = 5, skip: int = 0):
...
def get_vehicle_by_id(id: int, user: int = Depends(get_user), db: Session = Depends(get_db)):
...
def create_vehicle(vehicle: schemas.VehicleBase, user: int = Depends(get_user), db: Session = Depends(get_db)):
...
def update_vehicle_by_id(id: int, vehicle: schemas.VehicleUpdate, user: int = Depends(get_user), db: Session = Depends(get_db)):
...
def update_vehicle_by_num_plate(num_plate: str, vehicle: schemas.VehicleUpdate, user: int = Depends(get_user), db: Session = Depends(get_db)):  #TODO: proper validate num_plate entry
