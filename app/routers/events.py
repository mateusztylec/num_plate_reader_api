...
@router.get("/{id}", response_model=list[Event], status_code=status.HTTP_200_OK)
...
def get_events_by_vehicle_id(id: int, db: Session = Depends(get_db), limit: int = 5, skip: int = 0):
    events = db.query(models.Event).filter(models.Event.vehicle_id == id).limit(limit).offset(skip).all()
    if not events:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail=f"Vehicle with id: {id} does not exist!")
    return events


@router.get("/vehicles/plates/{num_plate}", response_model=list[Event], status_code=status.HTTP_200_OK)
def get_events_by_vehicle_id(num_plate: str, db: Session = Depends(get_db), limit: int = 5, skip: int = 0):
    num_plate = num_plate.replace(" ", "")
    events = db.query(models.Event).join(models.Vehicle).filter(models.Vehicle.num_plate == num_plate).limit(limit).offset(skip).all()
    if not events:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail=f"Vehicle with num_plate: {num_plate} does not exist!")
    return events
...
def delete_event_by_id(id, db: Session = Depends(get_db)):
    ...
    if not event_delete.first():
        raise HTTPException(status.HTTP_404_NOT_FOUND, f"Event with id {id} does not exist")
